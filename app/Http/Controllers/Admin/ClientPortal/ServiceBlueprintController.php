...
class ServiceBlueprintController extends Controller
{
    ...
    public function update(ServiceBlueprintVersion $version, Request $request): JsonResponse
    {
        ...
        $data = $request->validate([
            ...
            'folders' => ['array'], 'folders.*.client_key' => ['required', 'string', 'max:100'], 'folders.*.parent_client_key' => ['nullable', 'string', 'max:100'], 'folders.*.name' => ['required', 'string', 'max:150', 'not_in:.,..'], 'folders.*.client_visible' => ['required', 'boolean'],
        ]);
        DB::transaction(function () use ($version, $data) {
            ...
            while($pending->isNotEmpty()){ $progress=false; foreach($pending as $index=>$input){$parent=$input['parent_client_key']??null;if($parent&&!isset($map[$parent]))continue;$folder=$version->folders()->create(['parent_id'=>$parent?$map[$parent]:null,'name'=>$input['name'],'client_visible'=>$input['client_visible'],'sort_order'=>$index]);$map[$input['client_key']]=$folder->id;$pending->forget($index);$progress=true;} if(!$progress)abort(422,'Folder tree contains a missing or circular parent.'); }
        });
